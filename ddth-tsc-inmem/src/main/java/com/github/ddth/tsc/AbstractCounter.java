...
public abstract class AbstractCounter implements ICounter {
	 ...
	 */
	@Override
	public abstract DataPoint get(long timestampMs, DataPoint.Type type,
			int steps);
		...
		SortedSet<DataPoint> result = new TreeSet<DataPoint>(
				new Comparator<DataPoint>() {
					...
					public int compare(DataPoint block1, DataPoint block2) {
						return Longs.compare(block1.timestamp(),
								block2.timestamp());
					}
				});
	...
	public DataPoint[] getSeries(long timestampStartMs, long timestampEndMs,
			int steps) {
		return getSeries(timestampStartMs, timestampEndMs, steps,
				DataPoint.Type.SUM);
	}
	...
	public DataPoint[] getSeries(long timestampStartMs, long timestampEndMs,
		...
		for (DataPoint org : origin) {
			...
			if (block.type() == DataPoint.Type.NONE
					&& org.type() != DataPoint.Type.NONE) {
				block.type(type);
			}
			block.add(org);
